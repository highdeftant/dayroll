...
pub(super) fn draw_calendar_widget(selected_day: NaiveDate) -> Paragraph<'static> {
    ...
    Paragraph::new(lines)
        ...
        .block(
            Block::default()
                .title(Line::from(vec![
                    Span::styled(" CALENDAR ", bar_style().add_modifier(Modifier::BOLD)),
                    Span::styled(" scope ", chip_style(C_INFO, C_PANEL)),
                ]))
                .borders(Borders::ALL)
...
pub(super) fn draw_tasks_widget(
    area: Rect,
